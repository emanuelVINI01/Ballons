...
object Reflections {
    ...
    private fun expandVariables(name: String): String {
        ...
        while (matcher.find()) {
            ...
            replacement = if ("nms".equals(variable, ignoreCase = true)) NMS_PREFIX else if ("obc".equals(
                    variable,
                    ignoreCase = true
                )
            ) OBC_PREFIX else if ("version".equals(
                    variable,
                    ignoreCase = true
                )
            ) VERSION else throw IllegalArgumentException(
    ...
    fun getMethodSimply(clazz: Class<*>, method: String): Method? {
        for (m in clazz.getMethods()) if (m.name == method) return m
        return null
    }
    ...
    fun getTypedMethod(
        ...
        for (method in clazz.getDeclaredMethods()) {
            if ((methodName == null || method.name == methodName) && returnType == null || method.returnType == returnType && method.parameterTypes.contentEquals(
                    params
                )
            ) {
